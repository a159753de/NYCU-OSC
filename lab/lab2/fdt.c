...
// return 節點相對於 fdt 起始位址的 offset，如果找不到節點就回傳 -1
/** 作法：
 * 1. 直接跳到 struct block base address，開始往下走
 * 2. 比較 token
 */
int fdt_path_offset(const void *fdt, const char *path)
{
    uart_puts("Searching for node: ");
    uart_puts(path);
    uart_puts("\n");
    ...
    const uint32_t *struct_block_base = (const uint32_t *)((const char *)fdt + off_dt_struct); // 將 void * fdt 轉成 char *，這樣後續做地址運算都會以 1 byte 為單位，然後加上 off_dt_struct 的值，最後再轉成 uint32_t* 的指標，接下來的運算都是以 4 bytes 為單位
    ...
    const uint32_t *p = struct_block_base; // 建立指標 p 來遍歷 struct block，初始值為 struct_block 的起始位址

    /** 用一個 array 存放目前走到的 node path */
    char cur_node[1024];
    int cur_node_size = 1024;
    initPath(cur_node, cur_node_size); // 初始化 cur_node，第一個字元是 '/'，後面都是 '\0'
    ...
    while (1)
    {
        ...
        {
            const char *nodeName = (const char *)(p + 1);
            int nodeNameLen = strlen(nodeName);
            const uint32_t *next_p = (const uint32_t *)align_up(nodeName + nodeNameLen + 1, 4); // 將 nodeName + nodeNameLen 的位址對齊到 4 bytes，+1 是因為字元結尾是 '\0'，所以要加上 1 個 byte 的空間來存放

            // 先將 nodeName 放入 array 中
            for (int i = 1; i < nodeNameLen; i++)
            {
                cur_node[strlen(cur_node)] = nodeName[i];
            }
            cur_node[strlen(cur_node)] = '/'; // 在 nodeName 後面加上 '/'，這樣就可以繼續往下比對下一個 node
        }
    }
